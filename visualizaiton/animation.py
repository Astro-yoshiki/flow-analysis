...
def graph(*args):
    ...
    _T, _U, _V = plotter.predict_all_cell(params=_params, masking=True, return_prediction=True)
...
X, Y = plotter.make_grid(0.0, 111.5, 60, 135.0, 293.0, 60, return_grid=True)
# X, Y = plotter.make_grid(0.0, 76.2, 30, 135.0, 239.5, 30, return_grid=True)
T, U, V = plotter.predict_all_cell(params=params, masking=True, return_prediction=True)
...
scale_t.grid(row=2, column=0)

# 温度のテキスト
text_t = tk.Label(frame_2, text="Temperature [K]")
text_t.grid(row=1, column=0)
...
scale_coil_position.grid(row=8, column=0)

# コイル位置のテキスト
text_coil_position = tk.Label(frame_2, text="Coil position [mm]")
text_coil_position.grid(row=7, column=0)
...
label_coil_position.grid(row=8, column=1)
...
scale_r_down.grid(row=14, column=0)

# r_downのテキスト
text_r_down = tk.Label(frame_2, text="Lower insulation radius[mm]")
text_r_down.grid(row=13, column=0)
...
label_r_down.grid(row=14, column=1)
